/*
 * Slurm Rest API
 *
 * API to access and control Slurm DB.
 *
 * API version: dbv0.0.37
 * Contact: sales@schedmd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package slurmdbrest

import (
	"encoding/json"
)

// Dbv0037JobWckey Job assigned wckey details
type Dbv0037JobWckey struct {
	// Job assigned wckey
	Wckey *string `json:"wckey,omitempty"`
	// wckey flags
	Flags *[]string `json:"flags,omitempty"`
}

// NewDbv0037JobWckey instantiates a new Dbv0037JobWckey object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDbv0037JobWckey() *Dbv0037JobWckey {
	this := Dbv0037JobWckey{}
	return &this
}

// NewDbv0037JobWckeyWithDefaults instantiates a new Dbv0037JobWckey object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDbv0037JobWckeyWithDefaults() *Dbv0037JobWckey {
	this := Dbv0037JobWckey{}
	return &this
}

// GetWckey returns the Wckey field value if set, zero value otherwise.
func (o *Dbv0037JobWckey) GetWckey() string {
	if o == nil || o.Wckey == nil {
		var ret string
		return ret
	}
	return *o.Wckey
}

// GetWckeyOk returns a tuple with the Wckey field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Dbv0037JobWckey) GetWckeyOk() (*string, bool) {
	if o == nil || o.Wckey == nil {
		return nil, false
	}
	return o.Wckey, true
}

// HasWckey returns a boolean if a field has been set.
func (o *Dbv0037JobWckey) HasWckey() bool {
	if o != nil && o.Wckey != nil {
		return true
	}

	return false
}

// SetWckey gets a reference to the given string and assigns it to the Wckey field.
func (o *Dbv0037JobWckey) SetWckey(v string) {
	o.Wckey = &v
}

// GetFlags returns the Flags field value if set, zero value otherwise.
func (o *Dbv0037JobWckey) GetFlags() []string {
	if o == nil || o.Flags == nil {
		var ret []string
		return ret
	}
	return *o.Flags
}

// GetFlagsOk returns a tuple with the Flags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Dbv0037JobWckey) GetFlagsOk() (*[]string, bool) {
	if o == nil || o.Flags == nil {
		return nil, false
	}
	return o.Flags, true
}

// HasFlags returns a boolean if a field has been set.
func (o *Dbv0037JobWckey) HasFlags() bool {
	if o != nil && o.Flags != nil {
		return true
	}

	return false
}

// SetFlags gets a reference to the given []string and assigns it to the Flags field.
func (o *Dbv0037JobWckey) SetFlags(v []string) {
	o.Flags = &v
}

func (o Dbv0037JobWckey) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Wckey != nil {
		toSerialize["wckey"] = o.Wckey
	}
	if o.Flags != nil {
		toSerialize["flags"] = o.Flags
	}
	return json.Marshal(toSerialize)
}

type NullableDbv0037JobWckey struct {
	value *Dbv0037JobWckey
	isSet bool
}

func (v NullableDbv0037JobWckey) Get() *Dbv0037JobWckey {
	return v.value
}

func (v *NullableDbv0037JobWckey) Set(val *Dbv0037JobWckey) {
	v.value = val
	v.isSet = true
}

func (v NullableDbv0037JobWckey) IsSet() bool {
	return v.isSet
}

func (v *NullableDbv0037JobWckey) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDbv0037JobWckey(val *Dbv0037JobWckey) *NullableDbv0037JobWckey {
	return &NullableDbv0037JobWckey{value: val, isSet: true}
}

func (v NullableDbv0037JobWckey) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDbv0037JobWckey) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


