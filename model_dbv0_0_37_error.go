/*
 * Slurm Rest API
 *
 * API to access and control Slurm DB.
 *
 * API version: dbv0.0.37
 * Contact: sales@schedmd.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package slurmdbrest

import (
	"encoding/json"
)

// Dbv0037Error struct for Dbv0037Error
type Dbv0037Error struct {
	// Error number
	Errno *int32 `json:"errno,omitempty"`
	// Error message
	Error *string `json:"error,omitempty"`
}

// NewDbv0037Error instantiates a new Dbv0037Error object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDbv0037Error() *Dbv0037Error {
	this := Dbv0037Error{}
	return &this
}

// NewDbv0037ErrorWithDefaults instantiates a new Dbv0037Error object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDbv0037ErrorWithDefaults() *Dbv0037Error {
	this := Dbv0037Error{}
	return &this
}

// GetErrno returns the Errno field value if set, zero value otherwise.
func (o *Dbv0037Error) GetErrno() int32 {
	if o == nil || o.Errno == nil {
		var ret int32
		return ret
	}
	return *o.Errno
}

// GetErrnoOk returns a tuple with the Errno field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Dbv0037Error) GetErrnoOk() (*int32, bool) {
	if o == nil || o.Errno == nil {
		return nil, false
	}
	return o.Errno, true
}

// HasErrno returns a boolean if a field has been set.
func (o *Dbv0037Error) HasErrno() bool {
	if o != nil && o.Errno != nil {
		return true
	}

	return false
}

// SetErrno gets a reference to the given int32 and assigns it to the Errno field.
func (o *Dbv0037Error) SetErrno(v int32) {
	o.Errno = &v
}

// GetError returns the Error field value if set, zero value otherwise.
func (o *Dbv0037Error) GetError() string {
	if o == nil || o.Error == nil {
		var ret string
		return ret
	}
	return *o.Error
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Dbv0037Error) GetErrorOk() (*string, bool) {
	if o == nil || o.Error == nil {
		return nil, false
	}
	return o.Error, true
}

// HasError returns a boolean if a field has been set.
func (o *Dbv0037Error) HasError() bool {
	if o != nil && o.Error != nil {
		return true
	}

	return false
}

// SetError gets a reference to the given string and assigns it to the Error field.
func (o *Dbv0037Error) SetError(v string) {
	o.Error = &v
}

func (o Dbv0037Error) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Errno != nil {
		toSerialize["errno"] = o.Errno
	}
	if o.Error != nil {
		toSerialize["error"] = o.Error
	}
	return json.Marshal(toSerialize)
}

type NullableDbv0037Error struct {
	value *Dbv0037Error
	isSet bool
}

func (v NullableDbv0037Error) Get() *Dbv0037Error {
	return v.value
}

func (v *NullableDbv0037Error) Set(val *Dbv0037Error) {
	v.value = val
	v.isSet = true
}

func (v NullableDbv0037Error) IsSet() bool {
	return v.isSet
}

func (v *NullableDbv0037Error) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDbv0037Error(val *Dbv0037Error) *NullableDbv0037Error {
	return &NullableDbv0037Error{value: val, isSet: true}
}

func (v NullableDbv0037Error) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDbv0037Error) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


